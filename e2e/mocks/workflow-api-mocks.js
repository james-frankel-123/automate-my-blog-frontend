...
async function installWorkflowMocksBase(page, options = {}) {
  ...
  const patterns = [
    ...
    { path: '/api/v1/auth/register', method: 'POST', body: () => json({ success: true, accessToken: fakeJWT(), refreshToken: fakeJWT(), user: MOCK_USER }) },
    { path: '/api/v1/auth/login', method: 'POST', body: () => json({ success: true, accessToken: fakeJWT(), refreshToken: fakeJWT(), user: MOCK_USER }) },
