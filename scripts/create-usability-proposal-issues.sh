#!/usr/bin/env bash
# Create GitHub issues from docs/GITHUB_ISSUES_FROM_USABILITY_PROPOSAL.md (usability proposal, PR #47).
# Run from repo root. Requires: gh CLI, gh auth login.
...
echo "Creating 6 usability proposal issues..."
echo ""
...
BODY1
gh issue create --title "[UX] One consistent voice across the app so it feels like one helpful person" --body-file "$TMP"
...
BODY2
gh issue create --title "[UX] Suggest next steps so the app feels one step ahead" --body-file "$TMP"
...
BODY3
gh issue create --title "[UX] Show what's happening during analysis and generation (no black-box waiting)" --body-file "$TMP"
...
BODY4
gh issue create --title "[UX] Light motion and transitions so the flow feels continuous" --body-file "$TMP"
...
BODY5
gh issue create --title "[UX] One consistent place for hints, empty states, and errors" --body-file "$TMP"
...
BODY6
gh issue create --title '[UX] Quick wins: copy pass + "why we suggested this" + one anticipatory line + stagger/animate' --body-file "$TMP"
