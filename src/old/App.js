...
const AppContent = () => {
  ...
  // MIGRATED: All workflow state variables moved to WorkflowContainer.js
  const [currentStep, setCurrentStep] = useState(0); // MIGRATED: currentStep state
  const [showAuthModal, setShowAuthModal] = useState(false); // MIGRATED: showAuthModal state
  const [authContext, setAuthContext] = useState(null); // MIGRATED: authContext state // Track where auth modal was opened from
  const [selectedTopic, setSelectedTopic] = useState(null); // MIGRATED: selectedTopic state
  const [generatedContent, setGeneratedContent] = useState(''); // MIGRATED: generatedContent state
  const [isLoading, setIsLoading] = useState(false); // MIGRATED: isLoading state
  const [websiteUrl, setWebsiteUrl] = useState(''); // MIGRATED: websiteUrl state
  const [scanningMessage, setScanningMessage] = useState(''); // MIGRATED: scanningMessage state
  const [editingStep, setEditingStep] = useState(null); // MIGRATED: editingStep state
  const [previewMode, setPreviewMode] = useState(false); // MIGRATED: previewMode state
  const [selectedCMS, setSelectedCMS] = useState(null); // MIGRATED: selectedCMS state
  const [expandedSteps, setExpandedSteps] = useState([]); // MIGRATED: expandedSteps state
  const [analysisCompleted, setAnalysisCompleted] = useState(false); // MIGRATED: analysisCompleted state
  const [strategyCompleted, setStrategyCompleted] = useState(false); // MIGRATED: strategyCompleted state
  const [blogGenerating, setBlogGenerating] = useState(false); // MIGRATED: blogGenerating state
  
  // MIGRATED: Post state and content strategy management moved to WorkflowContainer.js
  const [postState, setPostState] = useState('draft'); // MIGRATED: postState // 'draft', 'exported', 'locked'
  const [contentStrategy, setContentStrategy] = useState({ // MIGRATED: contentStrategy state
  ...
  }); // END MIGRATED: contentStrategy state
  const [customFeedback, setCustomFeedback] = useState(''); // MIGRATED: customFeedback state
  const [showStrategyGate, setShowStrategyGate] = useState(false); // MIGRATED: showStrategyGate state
  const [showExportWarning, setShowExportWarning] = useState(false); // MIGRATED: showExportWarning state
  
  // MIGRATED: Change tracking for regeneration moved to WorkflowContainer.js
  const [previousContent, setPreviousContent] = useState(''); // MIGRATED: previousContent state
  const [showChanges, setShowChanges] = useState(false); // MIGRATED: showChanges state
  
  // MIGRATED: Customer strategy selection moved to WorkflowContainer.js
  const [selectedCustomerStrategy, setSelectedCustomerStrategy] = useState(null); // MIGRATED: selectedCustomerStrategy state
  const [strategySelectionCompleted, setStrategySelectionCompleted] = useState(false); // MIGRATED: strategySelectionCompleted state
  
  // MIGRATED: Web search research insights moved to WorkflowContainer.js
  const [webSearchInsights, setWebSearchInsights] = useState({ // MIGRATED: webSearchInsights state
    brandResearch: null,
    keywordResearch: null,
    researchQuality: 'basic' // 'basic', 'enhanced', 'premium'
  }); // END MIGRATED: webSearchInsights state
  
  // MIGRATED: Demo mode for testing moved to WorkflowContainer.js
  const [demoMode, setDemoMode] = useState( // MIGRATED: demoMode state
    process.env.REACT_APP_DEMO_MODE === 'true' || 
    window.location.search.includes('demo=true') ||
    localStorage.getItem('automyblog_demo_mode') === 'true'
  ); // END MIGRATED: demoMode state
  
  // MIGRATED: Step results storage moved to WorkflowContainer.js
  const [stepResults, setStepResults] = useState({ // MIGRATED: stepResults state
    websiteAnalysis: {
    ...
    finalContent: ''
  }); // END MIGRATED: stepResults state

  // No mock topics - only use real AI-generated content

  // MIGRATED: Steps configuration moved to WorkflowContainer.js
  const steps = [ // MIGRATED: steps configuration
  ...
  ]; // END MIGRATED: steps configuration

  // MIGRATED: Unified auth gate helper function moved to WorkflowContainer.js
  const requireAuth = (action, context = 'gate') => { // MIGRATED: requireAuth function
    if (!user) {
    ...
    return true;
  }; // END MIGRATED: requireAuth function

  // MIGRATED: CMS options configuration moved to WorkflowContainer.js
  const cmsOptions = [ // MIGRATED: cmsOptions configuration
    { 
      id: 'wordpress', 
      name: 'WordPress', 
      logo: '🔵',
  ...
  ]; // END MIGRATED: cmsOptions configuration

  // MIGRATED: Load trending topics useEffect moved to WorkflowContainer.js
  useEffect(() => { // MIGRATED: trending topics useEffect
    if (currentStep === 2) {
  ...
  }, [currentStep]); // END MIGRATED: trending topics useEffect
